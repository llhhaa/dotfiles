...
# Build output: Model | Dir | Context (line 1), then Branch on line 2
output="${C_ACCENT}${model}${C_GRAY} | 📁${git_root:-${dir}} | ${ctx}${C_RESET}"
...
if [[ -n "$transcript_path" && -f "$transcript_path" ]]; then
    ...
    plain_output="${model} | 📁${git_root:-${dir}} | xxxxxxxxxx ${pct}% of ${max_k}k tokens"
    plain_branch=""
    [[ -n "$branch" ]] && plain_branch="🔀${branch} ${git_status}"
    # Use the longer of the two lines for max_len
    [[ ${#plain_branch} -gt ${#plain_output} ]] && plain_output="$plain_branch"
