...
if [[ $(tmux ls) == *$SESSION* ]]; then
  ...
  tmux send-keys -t $SESSION:0 "cd ~/repos/ruby_tutorial/$(ls ~/repos/ruby_tutorial | sort -n | tail -n1)" C-m

  tmux new-window -n editor -t $SESSION
  tmux send-keys -t $SESSION:1 "cd ~/repos/ruby_tutorial/$(ls ~/repos/ruby_tutorial | sort -n | tail -n1)" C-m
