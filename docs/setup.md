...
| `lib/dotfiles/steps/skill_respec_step.rb` | Cross-platform. Writes each `skills/` definition into the managed directories of every *installed* harness (`~/.claude/skills`, `~/.config/opencode/skills`, plus `~/.config/opencode/command` for `slash: true` skills) as real files, overwriting same-named skills and pruning ones it previously wrote (via the `.skill-respec.json` manifest per dir). Never touches skills it doesn't own; skips harnesses whose CLI isn't installed. |
...
`SkillRespecStep` regenerates the per-harness copies (real files, gitignored where they land inside the repo). Setting `slash: true` in a harness override additionally generates a slash-invocable command file for that harness (`~/.config/opencode/command/<name>.md`) — released opencode surfaces `/`-menu entries through command files, not the skill frontmatter. To add a harness, add a `Harness` subclass and register it in `SkillRespec::HARNESSES`.
